...
def createNewUser(request, *args, **kwargs):
    ...
    while True:
        ...
        # firstname = request.POST.get('mobile', 'None')
        # lastname = request.POST.get('mobile', 'None')
...
#     except:
#         return log_in_view(request, 'User Not found')
#     products = Product.objects.all()
#     return render(request, 'index.html',
#                   {'products': products})
#     # return path('../products/', include('products.urls'))


# def validate():


# def showProduct(request, *args, **kwargs):
#     uname = request.POST.get('name', 'default')
#     upassword = request.POST.get('password', 'defalut')
#     user = authenticate(request, username=uname, password=upassword)
#     # if request.method == 'POST':
#     #     print('hello')
#     #     fm = index(request.POST)
#     #     nm = fm.clean_fields['name']
#     #     pk = fm.clean_fields['pid']
#     #     reg = Cart(productkey=pk, username=nm, quantity=0, total_price=0)
#     #     reg.save()
